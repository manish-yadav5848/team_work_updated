...
def load_file_in_raw(spark: SparkSession, job_arguments: RawJobArguments, file_parameters: FileParameters = None):
    ...
    #adHoc SSNChange for monthly
    excluded_files = [
        "EXN.XXADSUMT.XXADHSAX.JB",
        "EXN.XXADSUMT.XXADHSAX.NG",
        "EXN.XXADSUMT.XXADHSDX.NG",
        "EXN.XXADSUMT.XXADHSEX.JB",
        "EXN.XXADSUMT.XXADHSEX.NG",
        "EXN.XXADSUMT.XXADHSEX_2.JB",
        "EXN.XXADSUMT.XXADHSBX.JB",
        "EXN.XXADSUMT.XXADHSBX.NG",
        "EXN.XXADSUMT.XXADHSFX.NG",
        "EXN.XXADSUMT.XXADHSFX.JB",
        "EXN.XXADSUMT.XXADHSFX_2.JB",
        "SUPER_OMNI_NEWR_HISTBAL_NG_PRD.DAT",
        "SUPER_OMNI_NEWR_HISTBAL_JB_PRD.DAT"
    ]
    if (job_arguments.source_name == 'omni_7.4') and (file_parameters.file_load == 'incremental') and ('SEQHIST' not in job_arguments.file_name) and ('TRXH' not in job_arguments.file_name) and job_arguments.file_name not in excluded_files :
...
def ssn_handler(spark: SparkSession, job_arguments: RawJobArguments, file_parameters: FileParameters):
    ...
    ssn_output_df = read_from_catalog(
        ...
        database_name=database_name,
        table_name=table_name,
        condition_list=["admin_tran_code","CONCAT(tran_year,tran_month,tran_day)"],
        value_list=[["850", "851"],job_arguments.batch_date]
    )
